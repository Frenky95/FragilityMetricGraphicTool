...
while cont_tags <= len(tags) - 1:
    ...
    if prima_volta == 0:
        ...
        while cont_ttl_pv <= len(ntc) - 1:
            ...
            risultati_cloc_loc_v1 = subprocess.run(["pygount", local_rep_iminusone + ntc[cont_ttl_pv] ,"--format=summary"],stdout=subprocess.PIPE)
            output_cloc_ttl = risultati_cloc_loc_v1.stdout.decode('utf8').split('\r\n')
            cont_out_ttl = 0
            ...
            while cont_out_ttl < len(output_cloc_ttl):
                if output_cloc_ttl[cont_out_ttl].find("Java") != -1  and skip==0:
                    ...
                    output_cloc_ttl_filtrato = list(filter(lambda a: a != '', out_cloc_ttl_split))
                    ttl = ttl + int(output_cloc_ttl_filtrato[3])
        ...
        copy_properties = subprocess.call(['copy', '.\input\sonar-project.properties', dove_cop_vtag1],
                                          shell=True)
